...
static struct TPMAILBOX *tpSendMailBox;			//送信中のメールボックス
static struct TPMAILITEM *tpSendMailItem;		//送信メール
...
static TCHAR *GetPassword(struct TPMAILBOX *tpMailBox);
...
static BOOL SendMailString(HWND hWnd, SOCKET soc, struct TPMAILITEM *tpMailItem, TCHAR *ErrStr);
...
static BOOL SendMailProc(HWND hWnd, SOCKET soc, char *buf, TCHAR *ErrStr,
							  struct TPMAILITEM *tpMailItem, BOOL ShowFlag);
...
void HMAC_MD5(unsigned char *input, int len, unsigned char *key, int keylen, unsigned char *digest)
{
    ...
    tFillMemory(k_ipad, sizeof(k_ipad), 0x36);
    tFillMemory(k_opad, sizeof(k_opad), 0x5c);
    for(i = 0; i < keylen; i++){
        *(k_ipad + i) ^= *(key + i);
        *(k_opad + i) ^= *(key + i);
    }
...
/******************************************************************************
	...
	AUTH CRAM-MD5 のパスワードを生成する (RFC 2554)
...
static TCHAR *CreateAuthString(char *buf, TCHAR *User, TCHAR *Pass, TCHAR *ErrStr)
{
	...
	input = (char *)LocalAlloc(LMEM_FIXED, tstrlen(p) + 1);
	if(input == NULL){
#ifdef UNICODE
		LocalFree(key);
#endif
...
static TCHAR *CreatePlainString(TCHAR *User, TCHAR *Pass, TCHAR *ErrStr)
{
	...
	tmp = (char *)LocalAlloc(LPTR, len + 1);
	if(tmp == NULL){
#ifdef UNICODE
		LocalFree(c_user);
		LocalFree(c_pass);
	...
	buf = (char *)LocalAlloc(LPTR, len * 2 + 4);
	if(buf == NULL){
		LocalFree(tmp);
		lstrcpy(ErrStr, STR_ERR_MEMALLOC);
		return NULL;
...
/******************************************************************************
	...
	利用可能な SMTP AUTH の種類を取得 (RFC 2554)
...
static BOOL SendHeader(SOCKET soc, TCHAR *Head, TCHAR *Sendbuf, TCHAR *ErrStr)
{
	...
	buf = (TCHAR *)LocalAlloc(LMEM_FIXED, sizeof(TCHAR) * (lstrlen(Head) + 1 + lstrlen(Sendbuf) + 2 + 1));
	if(buf == NULL){
		lstrcpy(ErrStr, STR_ERR_MEMALLOC);
		return FALSE;
	}
...
/******************************************************************************
	...
	ヘッダと本文の送信 (RFC 822, RFC 2822, RFC 2045, RFC 2076)
...
static BOOL SendMailString(HWND hWnd, SOCKET soc, struct TPMAILITEM *tpMailItem, TCHAR *ErrStr)
{
	...
	if(tpSendMailBox->MailAddress != NULL && *tpSendMailBox->MailAddress != TEXT('\0')){
		...
		if(tpSendMailBox->UserName != NULL){
			p = (TCHAR *)LocalAlloc(LMEM_FIXED, sizeof(TCHAR) * (lstrlen(tpSendMailBox->UserName) + 1));
			if(p != NULL){
	...
	//Date
	GetTimeString(buf);
	if(SendDate == 1 && SendHeader(soc, TEXT(HEAD_DATE), buf, ErrStr) == FALSE){
		return FALSE;
	}
	NULLCHECK_FREE(tpMailItem->Date);
#ifdef UNICODE
	{
		char *cbuf, *dbuf;
		...
		if(cbuf != NULL){
			dbuf = (char *)LocalAlloc(LMEM_FIXED, BUF_SIZE);
			if(dbuf != NULL){
				DateConv(cbuf, dbuf);
	...
	if(SendMessageId == 1){
		//Message-Id
		NULLCHECK_FREE(tpMailItem->MessageID);
...
/******************************************************************************
	...
	SMTPの応答プロシージャ (RFC 821, RFC 2821)
							  ...
							  struct TPMAILITEM *tpMailItem, BOOL ShowFlag)
{
	...
	case SMTP_START:
		...
		//ドメインの抽出
		if(SendHelo == NULL || *SendHelo == TEXT('\0')){
		...
		if(SmtpAuthFlag == FALSE && tpSendMailBox->SmtpAuth == 1){
			...
			SmtpAuthFlag = FALSE;
			r = (ESMTP != 0) ? TEXT("EHLO ") : TEXT("HELO ");
			MailFlag = SMTP_HELO;
		}
		wbuf = (TCHAR *)LocalAlloc(LMEM_FIXED, sizeof(TCHAR) * (lstrlen(r) + lstrlen(p) + lstrlen(TEXT("\r\n")) + 1));
		if(wbuf == NULL){
			lstrcpy(ErrStr, STR_ERR_MEMALLOC);
			return FALSE;
		}
	...
	case SMTP_EHLO:
		...
		case AUTH_LOGIN:
			...
			p = (TCHAR *)LocalAlloc(LPTR, sizeof(TCHAR) * (lstrlen(user) * 2 + 4));
			if(p == NULL){
				lstrcpy(ErrStr, STR_ERR_MEMALLOC);
				return FALSE;
			}
			...
			wbuf = (TCHAR *)LocalAlloc(LMEM_FIXED, sizeof(TCHAR) * (lstrlen(TEXT("AUTH LOGIN \r\n")) + lstrlen(p) + 1));
			if(wbuf == NULL){
				LocalFree(p);
				lstrcpy(ErrStr, STR_ERR_MEMALLOC);
				return FALSE;
			}
			//AUTH LOGIN User[CRLF] を送信する
			TStrJoin(wbuf, TEXT("AUTH LOGIN "), p, TEXT("\r\n"), (TCHAR *)-1);
			SetSocStatusTextT(hWnd, wbuf, 1);
			if(TSendBuf(soc, wbuf) == -1){
				LocalFree(p);
				LocalFree(wbuf);
		...
		case AUTH_PLAIN:
			...
			wbuf = (TCHAR *)LocalAlloc(LMEM_FIXED, sizeof(TCHAR) * (lstrlen(TEXT("AUTH PLAIN \r\n")) + lstrlen(p) + 1));
			if(wbuf == NULL){
				LocalFree(p);
				lstrcpy(ErrStr, STR_ERR_MEMALLOC);
				return FALSE;
	...
	case SMTP_AUTHLOGIN:
		...
		if(wbuf != NULL){
			...
			r = GetPassword(tpSendMailBox);
			if(r == NULL || *r == TEXT('\0')){
				lstrcpy(ErrStr, STR_ERR_SOCK_NOPASSWORD);
				return FALSE;
			}
		}
		p = (TCHAR *)LocalAlloc(LPTR, sizeof(TCHAR) * (lstrlen(r) * 2 + 4));
		if(p == NULL){
			NULLCHECK_FREE(wbuf);
			lstrcpy(ErrStr, STR_ERR_MEMALLOC);
			return FALSE;
		...
		wbuf = (TCHAR *)LocalAlloc(LMEM_FIXED, sizeof(TCHAR) * (lstrlen(TEXT("\r\n")) + lstrlen(p) + 1));
		if(wbuf == NULL){
			LocalFree(p);
			lstrcpy(ErrStr, STR_ERR_MEMALLOC);
			return FALSE;
...
SOCKET SendMailItem(HWND hWnd, struct TPMAILBOX *tpMailBox, struct TPMAILITEM *tpMailItem, int EndMailFlag, TCHAR *ErrStr)
{
	...
	if(tpSendMailBox->SmtpIP == 0 || IPCache == 0){
		GetHostFlag = TRUE;
