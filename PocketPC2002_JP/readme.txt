...
インストールは、Pocket PC 2002 にnPOP.arm.CABをコピーして、Pocket PC 2002 でコ
ピーした nPOP.arm.CAB を開くと自動的にインストールします。インストールされる
フォルダは \Program Files\nPOP\ になります。

アンインストールは「アプリケーションの削除」でnPOPを選択して削除してください。
個人データは削除されないので、\Program Files\nPOP\ の中にある以下のファイルを
手動で削除してください。
	nPOP.ini
	Address.lst
	SaveBox.dat
...
上記例は、Subject: が [hoge-ML* のもののみを受信するが、From: が hoge@hoge.
hoge
のものは受信しないというフィルタになります。
...
メール表示では32K以上のメールを表示できませんが、外部ビューアを使うことで全て
表示することができます。外部ビューアはメール表示画面の編集メニューに「外部ビュー
ア」という項目を選択することで起動できます。また一覧でShiftキーを押しながらメー
ルを開くと外部ビューアで開きます。
...
を行います。Pocket PCで十字キーのみで未読メールを読み進めることができます。
...
以下は仕様です。
	...
	・Windows95,98,CEではメール表示、編集ともに32Kまでしか扱えません。
...
(C) Copyright 1999-2001 By Tomoaki Nakashima. All right reserved.
	http://www.nakka.com/
