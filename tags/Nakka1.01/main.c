...
static BOOL ExecFlag = FALSE;				//実行中
...
static struct TPMAILITEM *wkSendMailItem;	//送信用メールアイテム

int SelBox;									//選択中のメールボックス
static int RecvBox;							//送受信中のメールボックス
static int CheckBox;						//チェック中のメールボックス

struct TPMAILBOX *MailBox = NULL;			//メールボックス
struct TPMAILBOX *AddressBox = NULL;		//アドレス帳
int MailBoxCnt = 2;							//メールボックス数

static WNDPROC ListViewWindowProcedure;		//サブクラス用プロシージャ(ListView)

typedef BOOL (*PPROC)(HWND, SOCKET, char*, int, TCHAR*, struct TPMAILBOX*, BOOL);
...
extern TCHAR *FontName;
extern int FontSize;
extern int FontCharset;
...
extern TCHAR *LvFontName;
extern int LvFontSize;
extern int LvFontCharset;
...
extern int PopBeforeSmtpIsLoginOnly;
extern int PopBeforeSmtpWait;
...
extern int CheckEndExec;
extern int CheckEndExecNoDelMsg;
...
static BOOL SendMail(HWND hWnd, struct TPMAILITEM *tpMailItem, int EndMailFlag);
...
#define SAVEPATH	TEXT("\\My Documents\\")
	...
	AppDir = (TCHAR *)LocalAlloc(LPTR, sizeof(TCHAR) * BUF_SIZE);
	if(AppDir == NULL){
		return FALSE;
	}
...
void SetSocStatusTextT(HWND hWnd, TCHAR *buf, int Part)
{
	...
	if(PPCFlag == FALSE && RecvBox >= MAILBOX_USER && (MailBox + RecvBox)->Name != NULL){
		SetBuf = (TCHAR *)LocalAlloc(LMEM_FIXED, sizeof(TCHAR) * (lstrlen((MailBox + RecvBox)->Name) + lstrlen(buf) + 4));
		if(SetBuf == NULL){
			return;
		}
...
#endif
		if(SocLog == 1) SaveLog(AppDir, LOG_FILE, SetBuf);
		...
		SendDlgItemMessage(hWnd, IDC_STATUS, SB_SETTEXT, (WPARAM)Part, (LPARAM)buf);
#endif
		if(SocLog == 1) SaveLog(AppDir, LOG_FILE, buf);
	}
}
...
void SocketErrorMessage(HWND hWnd, TCHAR *buf, int BoxIndex)
{
	...
	if(SocIgnoreError == 1 && BoxIndex >= MAILBOX_USER){
		//受信エラーを無視する設定の場合
		return;
	}
	if(SendIgnoreError == 1 && BoxIndex == MAILBOX_SEND){
		//送信エラーを無視する設定の場合
		return;
	}
	if((BoxIndex == MAILBOX_SEND || BoxIndex >= MAILBOX_USER) &&
		...
		//アカウント名付きのタイトルの作成
		p = Title = (TCHAR *)LocalAlloc(LMEM_FIXED,
...
void SetMailMenu(HWND hWnd)
{
	...
	EnableMenuItem(hMenu, ID_MENUITEM_RAS_CONNECT,
		!(SocFlag & (MailBox + SelBox)->RasMode & !EnableLAN));
	EnableMenuItem(hMenu, ID_MENUITEM_RAS_DISCONNECT, EnableLAN);
...
#ifndef _WIN32_WCE
	SendDlgItemMessage(hWnd, IDC_TB, TB_ENABLEBUTTON, ID_MENUITEM_RECV,
		(LPARAM)MAKELONG(SocFlag & SaveBoxFlag & SendBoxFlag, 0));
	SendDlgItemMessage(hWnd, IDC_TB, TB_ENABLEBUTTON, ID_MENUITEM_ALLCHECK, (LPARAM)MAKELONG(SocFlag, 0));
	SendDlgItemMessage(hWnd, IDC_TB, TB_ENABLEBUTTON, ID_MENUITEM_EXEC,
		(LPARAM)MAKELONG(SocFlag & SaveBoxFlag, 0));
	SendDlgItemMessage(hWnd, IDC_TB, TB_ENABLEBUTTON, ID_MENUITEM_ALLEXEC, (LPARAM)MAKELONG(SocFlag, 0));
	SendDlgItemMessage(hWnd, IDC_TB, TB_ENABLEBUTTON, ID_MENUITEM_STOP, (LPARAM)MAKELONG(!SocFlag, 0));
	...
	EnableMenuItem(hMenu, ID_MENUITEM_DOWNMARK, !(SelFlag & SaveBoxFlag));
	EnableMenuItem(hMenu, ID_MENUITEM_DELMARK, !(SelFlag & SaveBoxFlag & SendBoxFlag));
	EnableMenuItem(hMenu, ID_MENUITEM_UNMARK, !(SelFlag & SaveBoxFlag));
...
static void FreeAllMailBox(void)
{
	...
	NULLCHECK_FREE(AddressBox);

	//すべてのメールボックスの解放
	for(i = 0; i < MailBoxCnt; i++){
		FreeMailBox((MailBox + i));
	}
	NULLCHECK_FREE(MailBox);
}
...
static LRESULT CALLBACK SubClassListViewProc(HWND hWnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_CHAR:
		if((TCHAR)wParam == TEXT(' ') && GetKeyState(VK_CONTROL) >= 0 && GetKeyState(VK_SHIFT) >= 0){
			if(ListView_GetItemCount(hWnd) == 0 && MoveAllMailBox == 1){
...
static LRESULT ListViewHeaderNotifyProc(HWND hWnd, LPARAM lParam)
{
	...
	case HDN_ITEMCLICK:
		...
		if(SelBox == MAILBOX_SAVE || LvAutoSort == 2){
			LvSortItem = LvSortFlag;
		}
		break;
	}
	return FALSE;
...
static BOOL InitWindow(HWND hWnd)
{
	...
	CSOBAR_BUTTONINFO ButtonInfo[] = {
		...
		2,						CSOBAR_BUTTON_SUBMENU_DOWN,	CSO_BUTTON_DISP, (-1),					NULL, TEXT("ﾒｰﾙ"),	NULL, CSOBAR_CODEPOS_CENTER, 1, (-1), (-1), (-1), (-1), 0, CLR_INVALID, CLR_INVALID, CLR_INVALID,			FALSE, FALSE,
	...
	Height = 0;
	i = 0;
	CheckMenuItem(SHGetSubMenu(hMainToolBar, ID_MENUITEM_FILE), ID_MENUITEM_LAN, (EnableLAN == 1) ? MF_CHECKED : MF_UNCHECKED);
	CheckMenuItem(SHGetSubMenu(hMainToolBar, ID_MENUITEM_MAIL), ID_MENUITEM_THREADVIEW, (LvThreadView == 1) ? MF_CHECKED : MF_UNCHECKED);
	PPCFlag = TRUE;
	MailMenuPos = 1;
	...
	i = 0;
	CheckMenuItem(GetSubMenu(hMainMenu, 0), ID_MENUITEM_LAN, (EnableLAN == 1) ? MF_CHECKED : MF_UNCHECKED);
	CheckMenuItem(GetSubMenu(hMainMenu, 1), ID_MENUITEM_THREADVIEW, (LvThreadView == 1) ? MF_CHECKED : MF_UNCHECKED);
	PPCFlag = TRUE;
	MailMenuPos = 1;
	...
	i = 0;
	CheckMenuItem(CommandBar_GetMenu(hToolBar, 0), ID_MENUITEM_LAN, (EnableLAN == 1) ? MF_CHECKED : MF_UNCHECKED);
	CheckMenuItem(CommandBar_GetMenu(hToolBar, 0), ID_MENUITEM_THREADVIEW, (LvThreadView == 1) ? MF_CHECKED : MF_UNCHECKED);
#endif
#else
	//Win32
	MailMenuPos = 3;
	...
	CheckMenuItem(GetMenu(hWnd), ID_MENUITEM_LAN, (EnableLAN == 1) ? MF_CHECKED : MF_UNCHECKED);
	CheckMenuItem(GetMenu(hWnd), ID_MENUITEM_THREADVIEW, (LvThreadView == 1) ? MF_CHECKED : MF_UNCHECKED);
#endif

	//フォントの作成
	if(LvFontName != NULL && *LvFontName != TEXT('\0')){
		hListFont = CreateEditFont(hWnd, LvFontName, LvFontSize, LvFontCharset);
	}
	//ViewとEditのフォント
	hEditFont = CreateEditFont(hWnd, FontName, FontSize, FontCharset);
...
static BOOL SaveWindow(HWND hWnd)
{
	...
	//ダイヤルアップの切断
	if(RasLoop == TRUE || RasEndDisCon == 1){
		RasDisconnect();
	}
	...
	for(i = 0; i < LV_COL_CNT; i++){
		LvColSize[i] = ListView_GetColumnWidth(GetDlgItem(hWnd, IDC_LISTVIEW), i);
	}
...
static BOOL SendMail(HWND hWnd, struct TPMAILITEM *tpMailItem, int EndMailFlag)
{
	...
	//ダイヤルアップ開始
	if(RasCon == 1 && SendMessage(hWnd, WM_RAS_START, BoxIndex, 0) == FALSE){
		ErrorSocketEnd(hWnd, MAILBOX_SEND);
		SetMailMenu(hWnd);
		return FALSE;
	}
	...
	if(SocLog == 1) SaveLogSep(AppDir, LOG_FILE, TEXT("send"));

	SetTimer(hWnd, ID_TIMEOUT_TIMER, TIMEOUTTIME * TimeoutInterval, NULL);
...
static BOOL RecvMailList(HWND hWnd, int BoxIndex, BOOL SmtpFlag)
{
	...
	if(SocLog == 1) SaveLogSep(AppDir, LOG_FILE, TEXT("recv"));

	RecvBox = BoxIndex;

	//ホスト名からIPアドレスを取得 (取得したIPは保存しておく)
	SwitchCursor(FALSE);
	if(tpMailBox->PopIP == 0 || IPCache == 0){
		GetHostFlag = TRUE;
	...
	g_soc = ConnectServer(hWnd,
		tpMailBox->PopIP, (unsigned short)tpMailBox->Port, ErrStr);
	if(g_soc == -1){
		ErrorSocketEnd(hWnd, BoxIndex);
		SocketErrorMessage(hWnd, ErrStr, BoxIndex);
		return FALSE;
...
static BOOL ExecItem(HWND hWnd, int BoxIndex)
{
	...
	if(SocLog == 1) SaveLogSep(AppDir, LOG_FILE, TEXT("exec"));
...
static void ReMessageItem(HWND hWnd)
{
	...
	if(SelBox == MAILBOX_SEND){
		...
		if(Edit_InitInstance(hInst, hWnd, SelBox,
			(struct TPMAILITEM *)ListView_GetlParam(hListView, i), EDIT_REPLY, 0) == EDIT_INSIDEEDIT){
#ifdef _WIN32_WCE
			ShowWindow(hWnd, SW_HIDE);
#endif
		}
...
static void EndSocketFunc(HWND hWnd)
{
	...
	if(ExecFlag == TRUE && ExecEndSound == 1){
		if(ExecEndSoundFile == NULL || *ExecEndSoundFile == '\0' ||
			sndPlaySound(ExecEndSoundFile, SND_ASYNC | SND_NODEFAULT) == FALSE){
#ifdef _WIN32_WCE_LAGENDA
...
static BOOL CheckEndAutoExec(HWND hWnd, int SocBox, int cnt, BOOL AllFlag)
{
	...
	if(AllFlag == TRUE){
		//巡回実行
		ShowError = TRUE;
		if(MailMarkCheck(hWnd, ((CheckEndExecNoDelMsg == 1) ? TRUE : FALSE), FALSE) == FALSE){
			ShowError = FALSE;
			return FALSE;
		}
		ShowError = FALSE;
		...
		if(CheckEndExecNoDelMsg == 1 && Item_GetNextDeleteItem((MailBox + SocBox), -1, NULL) != -1){
			ShowError = TRUE;
			if(MessageBox(hWnd, STR_Q_DELSERVERMAIL,
...
static void NewMail_Massage(HWND hWnd, int cnt)
{
	...
	if(cnt == 0){
		if(ShowError == FALSE && ShowNoMailMessage == 1 && ShowMsgFlag == FALSE
			&& AutoCheckFlag == FALSE){
			//メッセージボックスの表示
	...
	for(i = MAILBOX_USER; i < MailBoxCnt; i++){
		if(SelBox == i || (MailBox + i)->NewMail == FALSE ||
			((ListGetLine > 0 || ShowHeader == 1 || ListDownload == 1) &&
		...
		if((MailBox + i)->Name == NULL || *(MailBox + i)->Name == TEXT('\0')){
			...
			p = (TCHAR *)LocalAlloc(LMEM_FIXED, sizeof(TCHAR) * (lstrlen((MailBox + i)->Name) + 3));
			if(p != NULL){
	...
	if(NewMailSound == 1){
		if(NewMailSoundFile == NULL || *NewMailSoundFile == '\0' ||
			sndPlaySound(NewMailSoundFile, SND_ASYNC | SND_NODEFAULT) == FALSE){
#ifdef _WIN32_WCE_LAGENDA
			MessageBuzzer(0xFFFFFFFF);
#else
			MessageBeep(MB_ICONASTERISK);
	...
	if(ShowError == TRUE || ShowNewMailMessgae == 0 ||
...
#else
	//メッセージを表示しない設定か現在表示されているメールボックスの場合はメッセージボックスを出さない
	if(ShowError == TRUE || ShowNewMailMessgae == 0 || ShowMsgFlag == TRUE ||
		(AutoCheckFlag == FALSE && hWnd == GetForegroundWindow() && i == SelBox)){
		return;
	}
...
static LRESULT CALLBACK WndProc(HWND hWnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_CREATE:
...
#ifndef _WIN32_WCE
		SetWindowPos(hWnd, 0, MainRect.left, MainRect.top, MainRect.right, MainRect.bottom,
		...
		if(SocLog == 1) LogClear(AppDir, LOG_FILE);
		SwitchCursor(TRUE);
		...
		TrayIcon_Mail = LoadImage(hInst, MAKEINTRESOURCE(IDI_ICON_MAIN),
			IMAGE_ICON, SICONSIZE, SICONSIZE, 0);
		if(ShowTrayIcon == 1 && TrayIcon_Main != NULL){
	...
	case WM_COPYDATA:
		...
		if(ShowPass == 1 &&
			...
			Password != NULL && *Password != TEXT('\0') &&
			ConfirmPass(hWnd, Password) == FALSE){
			break;
		}
	...
	case COSH_EXECUTEAPP:
		//2重起動処理
		{
			...
			tZeroMemory(buf, sizeof(TCHAR) * BUF_SIZE);
	...
	case WM_EXITSIZEMOVE:
		if(IsWindowVisible(hWnd) != 0 &&
			...
			GetWindowRect(hWnd, (LPRECT)&MainRect);
	...
	case WM_TIMER:
		...
		case ID_RECV_TIMER:
			if(g_soc == -1){
				...
				if(AllCheck == FALSE){
					...
					if(ExecFlag == TRUE && CheckAfterUpdate == 1 && RecvBox != MAILBOX_SEND){
						//実行後チェック
						ExecFlag = FALSE;
		...
		case ID_SMTP_ONE_TIMER:
			...
			//メールの送信
			SendMail(hWnd, (struct TPMAILITEM *)wkSendMailItem, SMTP_SENDEND);
			wkSendMailItem = NULL;
			break;
		...
		case ID_SMTP_TIMER:
			...
			if((MailBox + CheckBox)->PopBeforeSmtp != 0 &&
				Item_GetNextMailBoxSendItem((MailBox + MAILBOX_SEND), -1, CheckBox) != -1){
				//POP before SMTP
				if(RasCon == 1 && SendMessage(hWnd, WM_RAS_START, CheckBox, 0) == FALSE){
					ErrorSocketEnd(hWnd, CheckBox);
					SetMailMenu(hWnd);
					break;
				}
				if(PopBeforeSmtpIsLoginOnly == 0 && NewMailCnt == -1) NewMailCnt = 0;
				RecvMailList(hWnd, CheckBox, (PopBeforeSmtpIsLoginOnly == 1) ? TRUE : FALSE);
				SmtpWait = PopBeforeSmtpWait / SMTPTIME;
			}
			break;
		...
		case ID_EXEC_TIMER:
			...
			if(CheckBox >= MAILBOX_USER && (MailBox + CheckBox)->CyclicFlag == 0 &&
				CheckAfterUpdate == 1 && ExecCheckFlag == FALSE){
				//実行後チェック
				RecvMailList(hWnd, CheckBox, FALSE);
				ExecCheckFlag = TRUE;
				break;
	...
	case WM_COMMAND:
		...
		case ID_MENUITEM_LAN:
			...
			CheckMenuItem(GetSubMenu(hMainMenu, 0), ID_MENUITEM_LAN, (EnableLAN == 1) ? MF_CHECKED : MF_UNCHECKED);
#else
			CheckMenuItem(CommandBar_GetMenu(GetDlgItem(hWnd, IDC_CB), 0), ID_MENUITEM_LAN, (EnableLAN == 1) ? MF_CHECKED : MF_UNCHECKED);
#endif
#else
			CheckMenuItem(GetMenu(hWnd), ID_MENUITEM_LAN, (EnableLAN == 1) ? MF_CHECKED : MF_UNCHECKED);
#endif
		...
		case ID_MENUITEM_THREADVIEW:
			...
			CheckMenuItem(GetSubMenu(hMainMenu, 1), ID_MENUITEM_THREADVIEW, (LvThreadView == 1) ? MF_CHECKED : MF_UNCHECKED);
#else
			CheckMenuItem(CommandBar_GetMenu(GetDlgItem(hWnd, IDC_CB), 0), ID_MENUITEM_THREADVIEW, (LvThreadView == 1) ? MF_CHECKED : MF_UNCHECKED);
#endif
#else
			CheckMenuItem(GetMenu(hWnd), ID_MENUITEM_THREADVIEW, (LvThreadView == 1) ? MF_CHECKED : MF_UNCHECKED);
#endif
		...
		case ID_MENUITEM_LISTINIT:
			...
			NULLCHECK_FREE((MailBox + SelBox)->LastMessageId);
		...
		case ID_MENUITEM_RECV:
			...
			AutoCheckFlag = FALSE;
			//ダイヤルアップ開始
			if(RasCon == 1 && SendMessage(hWnd, WM_RAS_START, i, 0) == FALSE){
				break;
			}
			AllCheck = FALSE;
			ExecFlag = FALSE;
		...
		case ID_MENUITEM_EXEC:
			...
			AutoCheckFlag = FALSE;
			//ダイヤルアップ開始
			if(RasCon == 1 && i >= MAILBOX_USER && SendMessage(hWnd, WM_RAS_START, i, 0) == FALSE){
				break;
			}
			AllCheck = FALSE;
			ExecFlag = TRUE;
			NewMailCnt = -1;
			if(CheckAfterUpdate == 1){
				NewMailCnt = 0;
				Init_NewMailFlag();
			}
			//マークを実行
		...
		case ID_MENUITEM_STOP:
			...
			KeyShowHeader = FALSE;
			NewMailCnt = -1;
			if(RasLoop == TRUE || (g_soc == -1 && RasCheckEndDisCon == 1)){
				RasDisconnect();
			}
	...
	case WM_SMTP_SENDMAIL:
		...
		wkSendMailItem = (struct TPMAILITEM *)lParam;
		NewMailCnt = -1;
		SmtpWait = 0;
		...
		if(CheckBox != -1 && (MailBox + CheckBox)->PopBeforeSmtp != 0){
			...
			AllCheck = TRUE;
			gSockFlag = TRUE;
			if(PopBeforeSmtpIsLoginOnly == 0) NewMailCnt = 0;
			RecvMailList(hWnd, CheckBox, (PopBeforeSmtpIsLoginOnly == 1) ? TRUE : FALSE);
			SmtpWait = PopBeforeSmtpWait / SMTPTIME;
		}
...
static HWND InitInstance(HINSTANCE hInstance, int CmdShow)
{
	...
	if(ShowTrayIcon == 1 && StartHide == 1){
		return hwndMain;
	}
#ifndef _WIN32_WCE
	if((CmdShow == SW_MINIMIZE || CmdShow == SW_SHOWMINIMIZED) && ShowTrayIcon == 1 && MinsizeHide == 1){
		return hwndMain;
	}
#endif
