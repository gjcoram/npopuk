...
このソフトはメールサーバ(POP3)上のメールを表示するものです。
一般的なメーラのリモートメール、仮想メールの機能をソフトにした感じのものです。
...
アーカイブはWin32用とWindows CE用と別れています。利用環境に合わせてお使いくだ
さい。
...
※ Windows CEは日本語版でないと正しく表示されないです。

インストールは適当なフォルダにコピーして実行するだけです。
設定は nPOP.exe と同じ場所に nPOP.ini を作成して書きみます。Windows CE 2.0以下
のバージョンでは My Documents のフォルダに設定ファイルやアカウント毎のファイル
を保存します。

Pocket PC(2002)版はCABファイルになっているので、Pocket PCにCABファイルをコピー
して、Pocket PCでコピーしたCABファイルを開くと自動的にインストールします。イン
ストールされるフォルダは \Program Files\nPOP\ になります。

l'agenda BE-500版はBE-500とPCを接続してからPC側でsetup.exeを起動すると自動的に
インストールします。
データファイルは本体にインストールした場合は \Nand Disk\My Documents\nPOP に保
存します。カードにインストールした場合は、\Storage Card\My Documents\nPOP に保
存します。
...
nPOPは、インターネットに接続できる環境というのが前提です。ダイヤルアップの場合
は、プロバイダに接続してからご使用してください。
...
基本的な動作は、
	...
	4. 「マークしたものを実行」にて受信マークの付いたメールを受信
	5. 「開く」で本文を読む
という感じです。(「新着取得時の行数」が 0 の場合)

「新着取得時の行数」を100などに設定してあると「新着チェック」で100行文の本文を
取得するため、そのメールを「開く」で本文を読むことが出来ます。
完全にメールを受信するには更に「受信用マーク」を付けて「マークしたものを実行」
を選択すると本文を完全に受信します。

「新着取得位置の初期化」を行うと一覧から削除したメールでサーバに残っているもの
を再取得することができます。
...
オプションの受信で全アカウントのメールの保存方法を設定できます。
	...
	・保存しない
		...
		を行うと前回最後に受信したものの次のメールより受信を行います。
		「新着取得位置の初期化」を行うと1番目のメールより受信を行うよ
		うになります。


メニューのアカウントの「上へ移動」と「下へ移動」は複数のアカウントの並び替えを
行う機能です。アカウントの上に登録されているものから順に巡回していくので巡回さ
せたい順番などに並び替えることができます。ただし保存箱と送信箱の移動は行えませ
ん。
...
受信マークの付いたメールは、メニューより「マークしたものを実行」を選択すること
で本文をすべて受信します。
...
なっている場合があります。その場合は本文をすべて受信するのでなく「新着取得時に
受信する行数」のみで本文を読むようにしてください。
...
イル名が表示されます。
メニューから添付ファイル名を選択すると保存確認のダイアログが表示され指定された
場所に保存します。
...
[オプション]の[チェック]の自動チェックは、指定の分毎に巡回を行うものです。
...
ダイヤルアップ未接続時に複数のアカウントのマークを設定したり、送信箱のメールに
送信マークをつけたりして、ダイヤルアップ接続後に一気にすべてのマークを実行する
という使い方ができます。
...
サーバから削除は、メールに「削除用マーク」を付加して、「マークしたものを実行」
することで削除できます。サーバからメールが削除されると一覧からも消えます。
...
（削除したものを再度読む場合は新着取得位置を初期化して先頭位置から一覧を取得し
てください）
...
イル一覧の画面にて指定できます。
Win32版の場合は添付ファイル一覧のウィンドウにファイルをドラッグ＆ドロップする
ことで追加を行うことも可能です。
...
て、「マークしたものを実行」を選択すると、複数件一度に送信することができます。
...
上記例は、From: が hoge@hoge.hoge のものは受信しないで、Subject: が [hoge-ML* 
のものは標準の受信動作をして、その他のメールには全て削除用にマークを付ける
というフィルタになります。
...
アドレス帳は簡単なものを付けています。個人情報を管理するようなものではなく、メー
ルアドレスのみを管理できる程度のものです。
...
オプションの「接続」のタブでダイヤルアップの設定を行うことが出来ます。
...
ダイヤルアップ設定は、
	...
	・終了時に切断
	・未接続時は自動チェックを行わない
	・ダイヤルアップ接続を切り替えるまでの待ち時間
があります。

切断のオプションはnPOPからの接続ではない場合は機能しません。
（nPOPが接続を行う前に既に接続を行っていた場合など）

「未接続時は自動チェックを行わない」はダイヤルアップを指定しているアカウントの
みに有効でLAN接続を指定しているアカウントは自動チェックを行います。


ダイヤルアップの接続先の設定はアカウント毎に設定を行います。アカウントの設定の
「接続」のタブで行います。

ダイヤルアップ設定は接続先毎にユーザ名とパスワードを保存するので、ユーザ名とパ
スワードを入力する前に接続先を選択しておいてください。
ダイヤルアップを指定して接続先を指定しない場合はダイヤルアップのオプションのみ
有効になります。(未接続時は自動チェックを行わない など)

LAN接続を指定している場合は、そのアカウントで送受信を行うときにダイヤルアップ
を使用しません。
メニューの「ファイル」->「ダイヤルアップ」の「LAN接続中」を選択するとダイヤル
アップの設定があってもダイヤルアップを行わないようになります。一時的に全てのア
カウントをLAN接続の設定にしたい場合に使用してください。

ダイヤルアップを指定している場合はそのアカウントで送受信を行うときに接続設定で
指定した接続先に接続を行います。
手動でメニューから「ダイヤルアップ接続」を選択した場合は表示しているアカウント
の接続設定でダイヤルアップ接続を開始します。

「既に接続中の場合は接続し直す」にチェックがある場合は nPOP で接続中でそのアカ
ウントの接続設定と違う設定の場合に一度切断して接続し直します。nPOP から以外の
接続の場合はこのオプションは効果が無いです。
接続し直すときは切断後にダイヤルアップ設定の「ダイヤルアップ接続を切り替えるま
での待ち時間」で指定した秒だけ待ってから接続を開始します。
（待ち時間を短くするとダイヤルアップを失敗する可能性があります）

接続設定は接続先、ユーザ名、パスワードを設定していないとエラーになります。
※ パスワードを省略することはできません。

ダイヤルアップの開始、進行状況はメール一覧画面のステータスバーに表示されます。
...
Ccや添付ファイルなど複数項目指定する場合は ,(カンマ) で区切ってください。
...
Windows 95,98,CEでは32K以上のメールを表示できませんが、外部ビューアを使うこと
で全て表示することができます。外部ビューアはメール表示画面の編集メニューに「外
部ビューア」という項目を選択することで起動できます。また一覧でShiftキーを押し
ながらメールを開くと外部ビューアで開きます。
...
Win32 や H/PC では、メール表示ウィンドウで Space でPageDownをしながら未開封
メールを表示していくことができます。他にAlt+↑↓でメールの移動などもできます。
(一覧画面ではアカウントの移動)

メール表示画面で「右端で折り返す」ようにしていると十字キーの右で Space の動作
(PageDownをしながら未開封メールを表示)をして、左で Shift + Space の動作(PageUp)
を行います。Pocket PCやPalm-size PCで十字キーのみで未読メールを読み進めること
ができます。
...
新着メールがあるときに鳴らすWAVファイルはiniファイルで設定できます。
	[GENERAL]
	NewMailSoundFile=
にWaveファイル名を指定します。指定がない場合はデフォルトの音が鳴ります。
Win32版はオプション画面で設定できます。
...
できます。
	[GENERAL]
	DataFileDir=
に既存のフォルダのパスを指定します。（Win32の場合はEXEファイルからの相対パスで
もいいです）
指定したフォルダにあらかじめファイルをコピーしておかないと空のファイルを作成し
てしまいます。後で上書きコピーでも構いませんが、プログラムを終了してからコピー
してください。
※ Windows CE 2.0以下の場合は、\My Documents\ が標準の保存先になっています。


Win32版では起動時やウィンドウ表示時にパスワードを要求することができます。
...
以下は仕様です。
	...
	・Windows 95,98,CEではメール表示、編集ともに32Kまでしか扱えません。
	  外部アプリケーションを利用すると32K以上扱えます。
	...
	・ダイヤルアップの切断に関するオプションはnPOPがダイヤルアップの接続を
	  行った場合のみ有効です。
