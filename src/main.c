...
struct parmdef {
	...
	{ PARM_MAILBOX,  NULL, 0,         OP_MAILBOX,  1, 0 },
...
static LRESULT CALLBACK WndProc(HWND hWnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_CLOSE:
		...
		if (op.CheckQueuedOnExit > 0 && gSendAndQuit == FALSE 
	...
	case WM_COMMAND: {
		...
		case ID_MENUITEM_QUIT:
			...
			if (SaveWindow(hWnd, FALSE, (gSendAndQuit == FALSE), FALSE) == FALSE) {
				gDoingQuit = FALSE;
				break;
			}
			save_flag = TRUE;
